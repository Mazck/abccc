...
module.exports = async function (api) {
	var threadModel, userModel, dashBoardModel, globalModel, paymentModel, gifcodeModel ,sequelize = null;
	switch (databaseType) {
		case "mongodb": {
			const spin = ora({
		...
		case "sqlite": {
			...
			spin.start();
			try {
				var { threadModel, userModel, dashBoardModel, globalModel, paymentModel, gifcodeModel, sequelize } = await require("../connectDB/connectSqlite.js")();
				process.stderr.clearLine = defaultClearLine;
				spin.stop();
	...
	const paymentData = await require("./paymentData.js")(databaseType, paymentModel, fakeGraphql);
	const GifcodeData = await require("./gifcodeData.js")(databaseType, gifcodeModel, fakeGraphql);
