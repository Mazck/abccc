...
/**
 * Tạo mã QR từ chuỗi VietQR và lưu về file
 * @param {string} qrData - Chuỗi mã QR VietQR
 * @param {string} [filename='vietqr.png'] - Tên file lưu
 * @returns {Promise<{ filePath: string, qrUrl: string }>}
 */
function createVietQRFromRaw(qrData, filename = 'vietqr.png') {
		...
		async function onStart() {
			...
			if (!command) {
				if (!hideNotiMessage.commandNotFound) {
					...
					// Tìm lệnh gần giống nhất
					const allCommands = Array.from(GoatBot.commands.keys());
					let closest = null;
					let minDistance = Infinity;

					for (const cmd of allCommands) {
						const distance = getLevenshtein(commandName, cmd);
					...
					const suggestion = minDistance <= 3 && closest
						? `\n👉 Có phải bạn muốn dùng lệnh: "${prefix}${closest}"?`
						: "";

					return await message.reply(
						commandName
							? utils.getText({ lang: langCode, head: "handlerEvents" }, "commandNotFound", commandName, prefix) + suggestion
