...
function logError(prefix, message) {
	...
	console.log(`${colors.redBright(`${characters} ${prefix}:`)}`, message);
	const error = Object.values(arguments).slice(2);
	for (let err of error) {
		if (typeof err == "object" && !err.stack)
			err = JSON.stringify(err, null, 2);
		console.log(`${colors.redBright(`${characters} ${prefix}:`)}`, err);
	}
}
...
module.exports = {
	...
	info: function (prefix, message) {
		...
		console.log(`${colors.greenBright(`${characters} ${prefix}:`)}`, message);
	},
	success: function (prefix, message) {
		...
		console.log(`${colors.cyanBright(`${characters} ${prefix}:`)}`, message);
	},
	master: function (prefix, message) {
		...
		console.log(`${colors.hex("#eb6734", `${characters} ${prefix}:`)}`, message);
	},
	dev: (...args) => {
