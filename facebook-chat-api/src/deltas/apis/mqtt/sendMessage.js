...
//Modified by Neth
//iloveyou wiegine
// @ChoruOfficial - Added typing indicator integration
...
module.exports = (defaultFuncs, api, ctx) => {
   ...
   */
  async function sendTypingIndicator(sendTyping, threadID) {
    let count_req = 0;
    const wsContent = {
      ...
      uploads.push(
        defaultFuncs
          ...
          .then(resData => {
            ...
            console.log(resData.payload.metadata[0])
  ...
  return async (msg, threadID, replyToMessage, callback) => {
    ...
    await sendTypingIndicator(true, threadID);
    ...
    try {
      ...
      // Kết thúc typing indicator sau khi gửi tin nhắn thành công
      await sendTypingIndicator(false, threadID);
      ...
      // Đảm bảo tắt typing indicator ngay cả khi có lỗi
      await sendTypingIndicator(false, threadID);
      throw error;
    }
  };
